...
## Parte B2 — novo layout

- Só apresentação: a lógica (datas automáticas com `diasManual`, `agruparPorJornada`, validador, `criarAcordo` com um único acordo) não muda. Modal `max-w-5xl`; em `lg:` duas colunas (passos à esquerda, "Resumo do acordo" sticky à direita); abaixo de `lg`, coluna única com o resumo depois dos passos.
- Passos em cards numerados: 1 "O que aconteceu?" (5 cards de situação, copy em `lib/acordos/situacoes.ts`), 2 "Onde e quem?" (posto + busca, Marcar todos/Limpar, elegíveis ativos primeiro; inelegíveis e não ativos em `<details>` "Não incluídos (N)"), 3 "Dados do acordo" (sub-passos a/b/c por template), 4 "Prazo limite" (só quando obrigatório ou já preenchido; senão link "Definir prazo limite (opcional)").
- Novos: `passo`, `situacao-cards`, `passo-funcionarios`, `motivo-chips` (chips por grupo do catálogo, "outro…" e nº do decreto), `dias-chips` (mini-cartões de calendário, clique remove, a conta "N dias × X min = Yh"), `prazo-limite` e `resumo-acordo` em `components/acordos/`; `campos-template.tsx` reestruturado mantendo `FormState`, `FORM_VAZIO` e `montarCampos`.
