...
import { registrarAdvertencia } from '@/app/(admin)/advertencias/actions'
...
export function ModalNovaAdvertencia({ open, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true)
    try {
      await registrarAdvertencia(data)
      form.reset()
      handleClose()
    } finally {
      setPending(false)
    ...
    <Dialog.Root open={open} onOpenChange={(isOpen) => { if (!isOpen) handleClose() }}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl">
          <Dialog.Title className="mb-4 text-lg font-semibold">Nova Advertência</Dialog.Title>
