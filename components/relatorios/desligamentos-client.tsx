...
import type { DesligamentoRow } from '@/app/(admin)/relatorios/desligamentos/actions'
...
const TODOS_MOTIVOS = Object.fromEntries(
...
function exportExcel(rows: DesligamentoRow[], mes: number, ano: number, MESES: string[]) {
  ...
  const allRows: XRow[] = [
    { data: [`Desligamentos — ${MESES[mes]} ${ano}`] },
    { data: [] },
  ...
  for (const r of rows) {
    allRows.push({ data: [
      ...
      r.supervisor,
      fmt(r.data_admissao),
      fmt(r.data_desligamento),
      fmtTempoCasa(r.tempo_casa_dias),
  ...
  XLSX.writeFile(wb, `desligamentos-${pad2(mes)}-${ano}.xlsx`)
}

interface Props { rows: DesligamentoRow[]; mes: number; ano: number; MESES: string[]; anos: number[] }

export function DesligamentosClient({ rows, mes, ano, MESES, anos }: Props) {
  ...
  return (
    <>
      <form method="get" className="flex flex-wrap items-end gap-3">
        ...
        <a href="/relatorios/desligamentos" className="flex h-9 items-center rounded-lg border border-gray-200 px-4 text-sm font-medium text-gray-500 hover:bg-gray-50">
          Limpar
        </a>
        ...
        <div className="ml-auto flex items-end gap-2">
          <button
            type="button"
            onClick={() => { setLoadingXlsx(true); try { exportExcel(filtered, mes, ano, MESES) } finally { setLoadingXlsx(false) } }}
