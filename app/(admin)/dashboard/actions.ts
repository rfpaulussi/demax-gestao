...
export async function buscarDadosSupervisor(supervisorId: string, dias = 7): Promise<DadosSupervisor> {
  ...
  type FuncRow = { id: string; nome: string; posto_id: string | null; status: string; funcao_id: string | null; eh_encarregado_volante: boolean | null }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: funcsRaw } = await (supabase as unknown as any)
    .from('funcionarios')
    .select('id, nome, posto_id, status, funcao_id, eh_encarregado_volante')
    .in('posto_id', postoIds)
    .in('status', ['ativo', 'atestado', 'ferias', 'afastado', 'faltante'])
  const funcs = (funcsRaw ?? []) as FuncRow[]
  ...
  const { data: funcoesDbRaw } = funcaoIds.length > 0
    ? await supabase.from('funcoes').select('id, nome').in('id', funcaoIds)
    : { data: [] as { id: string; nome: string }[] }
  const funcoesDb = (funcoesDbRaw ?? []) as { id: string; nome: string }[]
